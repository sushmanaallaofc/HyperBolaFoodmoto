...
class ChangePhoneNumber extends Component {
	...
	handleOtpInput = (otp) => {
        if (otp.length <= 6) {
            ...
            if(otp.length > 6){
                this.setState({ success_msg: true, validityMessage: "OTP must be 6 characters" });
                setTimeout(() => {
	...
	__sendOtp = (event) => {
		...
		if (isValidPhoneNumber && this.validator.fieldValid("phone")){
            if(this.state.phone !== this.props.user.data.phone) {
                ...
                this.setState({ success_msg: true, validityMessage: "Phone Number Already Exist" });
                setTimeout(() => {
                    this.setState({ success_msg: false });
                }, 2000);
            }
        } else {
            this.setState({ success_msg: true, validityMessage: "Invalid Phone Number" });
            setTimeout(() => {
                this.setState({ success_msg: false });
            }, 2000);
        }
    ...
    handleSubmit = (e) => {
        ...
        if (otp.length === 6) {
            ...
            this.__verifyOtpAndUpdatePhoneNumber();
        } else {
	...
	__verifyOtpAndUpdatePhoneNumber = () => {
        // Add a check to ensure OTP has the correct length
        if (this.state.otp.length !== 6) {
            this.setState({ success_msg: true, validityMessage: "Invalid OTP length" });
            setTimeout(() => {
                this.setState({ success_msg: false });
            }, 2000);
            return;
		...
		.then((response) => {
			...
			if(response.data.success){
				...
				this.setState({ success_msg: true, validityMessage: "Phone Number Updated" });
				setTimeout(() => {
					this.setState({ success_msg: false });
				}, 2000);
			} else {
                this.props.handlePopup();
                this.setState({ success_msg: true, validityMessage: "Invalid OTP" });
                setTimeout(() => {
					this.setState({ success_msg: false });
				}, 2000);
            }
			...
			<React.Fragment>
				...
				{this.state.success_msg && (
					<div className="auth-error mb-50">
						<div className="error-shake">{this.state.validityMessage}</div>
					</div>
				)}
				<Dialog
					fullWidth={true}
				...
				>
					<div className="container" style={{ borderRadius: "5px" }}>
						<React.Fragment>
							...
							<form onSubmit={this.handleSubmit} id="otpForm" className="hidden">
                                <div className="form-group px-15 pt-30">
                                    <input
                                        name="otp"
                                        style={{ border: "1px solid #ccc", padding: "20px" }}
                                        type="number"
                                        onChange={(e) => this.handleOtpInput(e.target.value)}
                                        className="form-control edit-address-input"
                                        autoFocus={true}
                                    />
                                    <button
                                        type="submit"
                                        className="btn btn-md btn-block text-white mt-20"
                                        style={{
                                            backgroundColor: localStorage.getItem("storeColor"),
                                            height: "3rem",
                                            textTransform: "uppercase",
