...
export const getMeatCategories = (id) => (dispatch, getState) => {
	return Axios.post(GET_MEAT_CATEGORIES_URL,{
			id: id
		})
		.then((response) => {
			const meat_categories = response.data;
			return dispatch({ type: GET_MEAT_CATEGORIES, payload: meat_categories });
		})
		.catch(function(error) {
			console.log(error);
		});
...
export const getRestaurantItems = (slug, categoryId = null) => (dispatch) => {
	Axios.post(GET_RESTAURANT_ITEMS_URL + "/" + slug, { categoryId: categoryId })
		.then((response) => {
...
export const getCategoryItems = (slug, categoryId) => (dispatch) => {
	Axios.post(GET_CATEGORY_ITEMS_URL, {
			slug: slug,
			categoryId: categoryId,
		})
		.then((response) => {
			const category_items = response.data;
			return dispatch({ type: GET_CATEGORY_ITEMS, payload: category_items });
		})
		.catch(function(error) {
			console.log(error);
		});
